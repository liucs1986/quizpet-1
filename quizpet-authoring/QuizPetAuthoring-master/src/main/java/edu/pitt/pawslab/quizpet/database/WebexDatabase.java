...
public class WebexDatabase extends JdbcDaoSupport {
	...
	public HashMap<Integer, StringBuilder> getUserAllTopics(int authorid){
		final String sql = "SELECT `ent_jquestion`.`QuestionID`, `ent_jquestion`.`Title` FROM `" + Setting.DBNAME + "`.`ent_jquestion` WHERE domain = ? and (AuthorId = ? or Privacy = '1')";
		final HashMap<Integer, StringBuilder> result = new HashMap<Integer, StringBuilder>();
		getJdbcTemplate().query(
				sql,
	...
	public Topic newTopic(final Topic newTopic){
		...
		getJdbcTemplate().update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				pStatement.setInt(5, newTopic.isPrivacy() == true ? 0 : 1);
		...
		return getJdbcTemplate().queryForObject(
				...
				new RowMapper<Topic>(){
					...
					public Topic mapRow(ResultSet rs, int rowNum) throws SQLException {
						...
						rsTopic.setPrivacy(rs.getInt(6) == 0);
						return rsTopic;
					}
				});
	}
		...
		return (ArrayList<Topic>) getJdbcTemplate().query(
				...
				new RowMapper<Topic>(){
					...
					public Topic mapRow(ResultSet rs, int rowNum) throws SQLException {
						...
						rsTopic.setPrivacy(rs.getInt(4) == 0);
						return rsTopic;
					}
				});
	}
	...
	public ArrayList<Quiz> blurSearch(StringBuilder keyword, Integer authorId){
		...
		final String sql = "SELECT `ent_jquiz`.`QuizID`, `ent_jquiz`.`Title`, `ent_jquiz`.`rdfIdDisplay`, `ent_jquiz`.`version`"
				...
				+ "WHERE Title LIKE ? AND (Privacy = 1 OR AuthorID = ?)";
...
//				+ "WHERE Title LIKE ?";
		...
		if(keyword.toString().equals("-1")){
			final String sql = "SELECT `a`.`QuizID`, `a`.`Title`, `a`.`rdfID` , `a`.`AuthorId`, `d`.`name` , `a`.`Description`"
					...
					+ "WHERE  d.id = a.AuthorId and a.QuizID = b.QuizID and  c.QuestionID = b.QuestionID and c.domain ='py' and (a.AuthorId = ? or a.Privacy = '1')";
			...
			return (ArrayList<Quiz>) getJdbcTemplate().query(
					...
					new RowMapper<Quiz>(){
						...
						public Quiz mapRow(ResultSet rs, int rowNum) throws SQLException {
							...
							rsQuestion.setDecp(new StringBuilder(rs.getString(6)));
		...
		final String sql = "SELECT `a`.`QuizID`, `a`.`Title`, `a`.`rdfID`, `a`.`AuthorId`, `d`.`name`, `a`.`Description`"
				...
				+ "WHERE  d.id = a.AuthorId and a.QuizID = b.QuizID and b.QuestionID = ? and (a.AuthorId = ? or a.Privacy = '1') ";
		System.out.println(sql);
		System.out.println(keyword.toString());
		return (ArrayList<Quiz>) getJdbcTemplate().query(
				sql,
...
//		
//		final String sql = "SELECT `a`.`QuizID`, `a`.`Title`, `a`.`rdfIdDisplay`, `a`.`version`"
//				+ " FROM `" + Setting.DBNAME + "`.`ent_jquiz` as a, `" + Setting.DBNAME + "`.`rel_question_quiz` as b "
//				+ "WHERE  a.QuizID = b.QuizID and b.QuestionID = ? ";
//		System.out.println(sql);
//		System.out.println(keyword.toString());
//		return (ArrayList<Quiz>) getJdbcTemplate().query(
//				sql,
//				new Object[]{keyword.toString()},
//				new RowMapper<Quiz>(){
//					@Override
//					public Quiz mapRow(ResultSet rs, int rowNum) throws SQLException {
//						Quiz rsQuestion = new Quiz();
...
//				});
		...
		return getJdbcTemplate().queryForObject(
				...
				new RowMapper<Quiz>(){
					...
					public Quiz mapRow(ResultSet rs, int rowNum) throws SQLException {
						...
						rsPythonQuiz.setPrivacy(rs.getInt("Privacy") > 0 ? false : true);
	...
	public Quiz newQuiz(final Quiz quiz){
		...
		getJdbcTemplate().update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				pStatement.setInt(9, quiz.isPrivacy() == true ? 0 : 1);
	...
	public HashMap<Integer, StringBuilder> getAllPyClasses(){
		final String sql = "SELECT * FROM `" + Setting.DBNAME + "`.`ent_class` where ClassName like  '%.py'";
		final HashMap<Integer, StringBuilder> result = new HashMap<Integer, StringBuilder>();
		getJdbcTemplate().query(
				sql,
				new RowMapper<Void>(){
	...
	public Integer newPyClass(final StringBuilder filename){
		final String sql = "INSERT INTO `" + Setting.DBNAME + "`.`ent_class` (`ClassName`) VALUES (?)";
		KeyHolder keyHolder = new GeneratedKeyHolder();
		getJdbcTemplate().update(new PreparedStatementCreator() {
			@Override
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
	...
	public Integer addConceptsByClassFile(String title, HashMap<StringBuilder, ArrayList<ConceptNode>> conceptListForOneQuiz){
		...
		while(iterator.hasNext()){
			...
			System.out.println("begin to insert concetp");
