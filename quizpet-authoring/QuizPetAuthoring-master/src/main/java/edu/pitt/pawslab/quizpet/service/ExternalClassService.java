		...
		if(!file.isEmpty()){
			try{
				...
				if(webexDatabase.checkClassFileName(fileName) > 0){
					...
					result.setMessage(new StringBuilder("Please change the filename."));
				}else{
					byte[] bytes = file.getBytes();
					BufferedOutputStream stream = new BufferedOutputStream(new FileOutputStream(new File(Setting.PYTHONCLASSFOLDER, fileName.toString())));
					stream.write(bytes);
					stream.close();
	...
	public ServerMessage getClassById(Integer classId){
		...
		if(webexDatabase.checkClassId(classId) > 0){
			...
			try{
				...
				BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(Setting.PYTHONCLASSFOLDER + "//" + filename.toString())));
