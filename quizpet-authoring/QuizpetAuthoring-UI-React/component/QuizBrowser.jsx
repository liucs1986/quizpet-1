    ...
    <tbody>
					{/*  {self.state.resultSetFilter.map(function(result,i){
						...
						{self.state.resultSetFilter.map(function(result,i){
							 ...
							 */}
							return <tr> <td className="text-center"><img   onClick={self.getBrowseClickHandler.bind(this,result.rdfId)}  src="resources/images/preview.png" data-pin-nopin="true" style={imgStyle}/></td> <td>{result.title}</td><td>{result.author}</td><td>{result.version}</td><td><button onClick={self.getCloneClickHandler.bind(this,result.quizId)} className="btn btn-default" >Clone</button></td><td>	<button className="btn btn-default" style={(!result.canModify)?inputStyle:inputStyleshow} onClick={self.getEditClickHandler.bind(this,result.quizId)} >Edit Question</button></td><td><button className="btn btn-default"  style={(!result.canModify)?inputStyle:inputStyleshow} onClick={self.props.handleEditConcept().bind(this,result.quizId,result.title,result.rdfId)} >Edit Concept</button></td></tr>
